...
const_mass = cat['mean nh3 mass']
const_mass = const_mass[1:len(cat['mean nh3 mass'])]
...
# This has to happen for the sake of line 62
...
###Plot CDF
pl.plot(np.log10(np.sort(const_mass)),np.log10(np.linspace(1,0,len(const_mass), endpoint=False)), label=r'By-eye Mean Flat Temp. (54K), $\alpha$ = -{:0.4f}'.format(pl_flat_alpha))
...
pl.plot(np.log10(np.sort(const_dendro)),np.log10(np.linspace(1,0,len(const_dendro), endpoint=False)), label=r'Dendrogram Mean Flat Temp. (66K), $\alpha$ = -{:0.4f}'.format(pl_flat_den_alpha))
...
pl.legend()
pl.title('CDF of By-eye vs. Dendrogram for MEAN Flat & Dynamic Temp')
pl.show()