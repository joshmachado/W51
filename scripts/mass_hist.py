...
pl.hist(const_dendro, bins=np.logspace(np.log(0.1),np.log(1000), 25), alpha=0.5, label='Const. Temp.')
pl.hist(ammon_dendro, bins=np.logspace(np.log(0.1),np.log(1000), 25), alpha=0.5, label='Ammonia Derived Temp.')
...
pl.ylabel('Counts')
pl.legend()
pl.title('Dendrogram Catalog Core Mass Distribution')
