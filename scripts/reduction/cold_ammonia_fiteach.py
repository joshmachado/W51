...
mask = pyfits.getdata('diff_mask.fits')
...
if os.path.exists('mosaic_m1_new.fits'):
    momentcubefile = pyfits.open('mosaic_m1_new.fits')
...
if astropy.version.major >= 2 or (astropy.version.major==1 and astropy.version.minor>=3):
    momentcubefile.writeto('mosaic_m1_new.fits',overwrite=True)
else:
    momentcubefile.writeto('mosaic_m1_new.fits',clobber=True)
...
guessfn = 'non_fourfour_guesscube.fits'
...
if os.path.exists('non_fourfour_parcube.fits'):
    cubes.load_model_fit('non_fourfour_parcube.fits', 6, fittype='ammonia', _temp_fit_loc=(94,250))
    ...
    cubes.fiteach(fittype='cold_ammonia', skip_failed_fits=False, guesses=guesses, maskmap=mask,
            ...
            use_nearest_as_guess=False, start_from_point=(120,135),
            multicore=12,
            errmap=errmap11)
    ...
    fitcubefile.writeto("non_fourfour_parcube.fits")
